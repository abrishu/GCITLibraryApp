...
function populateBookCopiesList(bookCopiesData){
	...
	if(bookCopiesData!=null){
		...
		$.each(bookCopiesData,function(index,bookCopy){
			...
			rowData+='<td><button type="button"'+ 'href="editBookCopies?bookId=' + this.book.id +'&branchId=' + this.branch.id  + '"data-toggle= "modal" data-target="#editModal" class="btn btn-success btn-sm btnEdit">Edit</button></td>';
			//Setting the delete button for the given author object
			rowData+='<td><button type="button"'+ 'href="delete?deleteAction=book&title=' +encodeURIComponent(this.title) + '&bookId=' + this.id   +  '" data-toggle="modal" data-target="#deleteModal" class="btn btn-danger btn-sm">Delete</button></td>'
...
function  loadPaginationData(searchStringBranch,searchStringBook){
	...
	$(document).ready(function(){
		...
		$('#searchStringBranch').change(function(){
			...
			loadPaginationData($(this).val(),$('#searchStringBook'));
		});
